...
const update = () => {
  ...
  const ctaOnScreen = ctaRect
    ? ctaRect.top < window.innerHeight && ctaRect.bottom > 0
    : false
  visible.value = pastHero && !ctaOnScreen
}

onMounted(() => {
  heroEl = document.querySelector('[data-fab-hero]')
  ctaEl = document.querySelector('[data-fab-cta]')
